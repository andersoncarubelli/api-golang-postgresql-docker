...
import (
	...
	"github.com/andersoncarubelli/api-golang-postgresql-docker/internal/repository/transaction"
...
func (r *repository) CreateUser(ctx context.Context, u *entity.UserEntity) error {
	...
	if err != nil {
		slog.Error("error to create user and address, rollback applied", "err", err)
		return err
	}
	return nil
}
...
func (r *repository) UpdateUser(ctx context.Context, u *entity.UserEntity) error {
	err := transaction.Run(ctx, r.db, func(q *sqlc.Queries) error {
		...
		err = q.UpdateUserAddress(ctx, sqlc.UpdateUserAddressParams{
			UserID:     u.ID,
	...
	if err != nil {
		slog.Error("error to update user and address, rollback applied", "err", err)
		return err
	}
	return nil
}
